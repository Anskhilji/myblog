...
//        $arr = $Options->get("intanal_links_settings");
        $arr = DB::table('internallinks')->first();
//        echo '<pre>';
//        print_r($arr);
//        echo '</pre>';
		if ($arr!=""){
			...
			if (isset($arr->type_of_links)){
				$this->type_of_link = $arr->type_of_links;
			}
			if (isset($arr->max_links_on_same_anchor_text)){
				$this->max_same_url_same_text  = ( is_numeric($arr->max_links_on_same_anchor_text)) ? $arr->max_links_on_same_anchor_text : $this->max_same_url_same_text;
			}
			if (isset($arr->max_links_on_different_anchor_text)){
				$this->max_same_url_different_text  = ( is_numeric($arr->max_links_on_different_anchor_text)) ? $arr->max_links_on_different_anchor_text : $this->max_same_url_different_text;
			}
			if (isset($arr->max_links_in_one_article)){
				$this->max_links = ( is_numeric($arr->max_links_in_one_article)) ? $arr->max_links_in_one_article : $this->max_links;
			}
			if (isset($arr->fixed_percent)){
				$this->link_len_type = (trim($arr->fixed_percent)=="") ? $this->link_len_type  :$arr->fixed_percent;
			}
			if (isset($arr->max_links_on_its_own_article)){
				$this->max_url_current_post = (trim($arr->max_links_on_its_own_article)=="") ? $this->max_url_current_post  :$arr->max_links_on_its_own_article;
			}
			if (isset($arr->max_links_limit)){
				$this->max_url_one_post = (trim($arr->max_links_limit)=="") ? $this->max_url_one_post  :$arr->max_links_limit;
			}
	...
	function allDBLinks($post_id = 0){
		$r = DB::table('posts')->select("id" , "slug" , "internal_tags")->get();
		$arr = array();
		foreach($r as $k=>$v){
			$id = $v->id;
			$tags = explode(",",$v->internal_tags);
			$slug = $v->slug;
			$link = $this->base_url."/".$slug."-2".$id;
			$arr[] = array(
					"id" => $id,
					"link" => $link,
					"tags" => $v->internal_tags
				);
		}
		return $arr;
	}
